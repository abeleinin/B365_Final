...
excluded_columns <- c("away", "date", "season", "winner", "home", "any_playoff_appearance")
...
importance_df <- data.frame(feature = row.names(importance_values), importance = importance_values[, 1])
...
# Using top 15 features in model and split data into testing and training data:
library(caret)
top_features <- head(importance_df, 15)$feature
selected_features_data <- all_seasons_data[, c(top_features, "any_playoff_appearance")]

split <- createDataPartition(selected_features_data$any_playoff_appearance, p = 0.7, list = FALSE)
training_set <- selected_features_data[split, ]
testing_set <- selected_features_data[-split, ]

training_set$any_playoff_appearance <- as.factor(training_set$any_playoff_appearance)
testing_set$any_playoff_appearance <- as.factor(testing_set$any_playoff_appearance)




# Train the Random Forest model using selected features:
rf_model_selected <- randomForest(any_playoff_appearance ~ ., data = training_set, ntree = 100)




# Evaluate the model with the testing set:
predictions_selected <- predict(rf_model_selected, newdata = testing_set)
confusionMatrix(predictions_selected, testing_set$any_playoff_appearance)
